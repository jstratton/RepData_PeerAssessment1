...
* We need to split the data into weekdays and weekends.

```{r weekdayvsweekend}
# Add a new column to the activity data to indicate the day of the week.
activity_imputed <- mutate(activity_imputed, day_name = weekdays(date))

# Turn these day names into a factor variable
activity_imputed$day_name <- factor(x = activity_imputed$day_name, levels = c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"), labels = c(rep("Weekday", times = 5), rep("Weekend", times = 2)))
